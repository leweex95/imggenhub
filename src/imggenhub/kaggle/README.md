...
Run the main script to perform all steps automatically:
    ...
    poetry run python -m imggenhub.kaggle.main --model-id "stabilityai/stable-diffusion-xl-base-1.0" --gpu --dest .
...
Or use the deploy script with GPU override to combine steps 1 & 2:
    ...
    poetry run python -m deploy --prompts-file prompts.json --gpu --model-id "stabilityai/stable-diffusion-xl-base-1.0"
