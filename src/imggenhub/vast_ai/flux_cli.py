"""CLI for running Flux AI pipelines on Vast.ai."""
...
def parse_args(argv: Optional[list[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Flux AI deployment on Vast.ai")
    ...
    parser.add_argument("--prompts-file", dest="prompts_file", type=str, default="config/prompts_flux.json", help="Remote prompts file path")
    ...
    parser.add_argument("--model-name", dest="model_name", type=str, default="black-forest-labs/FLUX.1-schnell", help="Model identifier")
    parser.add_argument("--guidance", dest="guidance", type=float, default=4.0, help="Guidance scale")
    parser.add_argument("--steps", dest="steps", type=int, default=20, help="Inference steps")
    parser.add_argument("--precision", dest="precision", type=str, default="fp16", choices=["fp32", "fp16", "int8", "int4"], help="Numerical precision")
    parser.add_argument("--expected-images", dest="expected_images", type=int, default=6, help="Images expected per run for logging")
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        execution = orchestrator.run_flux(
        ...
        print(f"Flux AI deployment failed: {exc}", file=sys.stderr)
        return 1

    result = execution.result
    print(f"Flux AI run finished with exit code {result.exit_code}")
    if result.log_file:
